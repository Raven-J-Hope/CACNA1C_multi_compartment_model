...
class DGGranuleLikeCell:
    PARAMETER_SPEC = {
        "soma": {
            ...
            "Cav22.gbar": 1e-5,
            "Cav32.gbar": 1e-5,
            "BK_TOTAL.gakbar": 3e-2,
            "BK_TOTAL.gabkbar": 3e-2,
            "SK2.gkbar": 5e-6,
            "HCN.gbar": 1e-4,
            "Kv42.gkbar": 1.5e-4,
            "Kv42b.gkbar": 1.5e-4,
        ...
        "ais": {
            "na8st.gbar": 0.000001 * 5.0,
            "ichan3.gnabar": 5e-2 * 2.0,
            "ichan3.gkfbar": 5e-4,
            "ichan3.gksbar": 5e-4,
            "ichan3.gkabar": 5e-4,
            "Cav12.gbar": 1e-5 * 2.5,
            ...
            "Cav22.gbar": 1e-5 * 2.5,
            "Cav32.gbar": 1e-5 * 2.5,
            "BK_TOTAL.gakbar": 3e-2 * 2.5,
            "BK_TOTAL.gabkbar": 3e-2 * 2.5,
            "SK2.gkbar": 5e-6 * 2.5,
            "HCN.gbar": 1e-4 * 2.5,
            "Kv42.gkbar": 1.5e-4 * 2.5,
            "Kv42b.gkbar": 1.5e-4 * 2.5,
            "Kv11.gkbar": 1e-5 * 2.5,
            "Kir21.gkbar": 1.5e-4 * 2.5,
            "Kv14.gkbar": 1e-5 * 2.5,
            "Kv21.gkbar": 3e-5 * 2.5,
            "Kv33.gkbar": 2e-2 * 2.5,
            "Kv34.gkbar": 2e-3 * 2.5,
            "Kv723.gkbar": 2e-9 * 2.5,
            "Caold.gtcabar": 1e-6 * 2.5,
            "Caold.gncabar": 1e-6 * 2.5,
            "Caold.glcabar": 1e-6 * 2.5,
            "Cabuffer.tau": 8.0,
            "Cabuffer.brat": 1.0,
            "Cav2_1.pcabar": 1e-5 * 2.5,
            "Cav2_1.vshift": 0.0,
        },
        "dend_prox": {
            "na8st.gbar": 0.000001 * 0.3,
            "ichan3.gnabar": 5e-2 * 0.3,
            "ichan3.gkfbar": 5e-4 * 0.3,
            "ichan3.gksbar": 5e-4 * 0.3,
            "ichan3.gkabar": 5e-4 * 0.3,
            "Cav12.gbar": 1e-5 * 1.8,
            "Cav13.gbar": 1e-9 * 1.8,
            "Cav22.gbar": 1e-5 * 1.8,
            "Cav32.gbar": 1e-5 * 1.8,
            "BK_TOTAL.gakbar": 3e-2 * 1.8,
            "BK_TOTAL.gabkbar": 3e-2 * 1.8,
            "SK2.gkbar": 5e-6 * 1.8,
            "HCN.gbar": 1e-4 * 1.8,
            "Kv42.gkbar": 1.5e-4 * 1.8,
            "Kv42b.gkbar": 1.5e-4 * 1.8,
            "Kv11.gkbar": 1e-5 * 1.8,
            "Kir21.gkbar": 1.5e-4 * 1.8,
            "Kv14.gkbar": 1e-5 * 1.8,
            "Kv21.gkbar": 3e-5 * 1.8,
            "Kv33.gkbar": 2e-2 * 1.8,
            "Kv34.gkbar": 2e-3 * 1.8,
            "Kv723.gkbar": 2e-9 * 1.8,
            "Caold.gtcabar": 1e-6 * 1.8,
            "Caold.gncabar": 1e-6 * 1.8,
            "Caold.glcabar": 1e-6 * 1.8,
            "Cabuffer.tau": 8.0,
            "Cabuffer.brat": 1.0,
            "Cav2_1.pcabar": 1e-5 * 1.8,
            "Cav2_1.vshift": 0.0,
        },
        "dend_dist": {
            "na8st.gbar": 0.000001 * 0.3,
    ...
    def __init__(self, name="dgcell", bk_split=None, channel_overrides=None):
        self.name = name
    ...
    def __init__(self, name="dgcell", bk_split=None, channel_overrides=None):
        ...
        self.spine_necks = []
        self.spine_xs = [] #store where spines attach
        self.channel_overrides = channel_overrides or {} #channel overrides for gui testing
...
#                    seg.BK.gabkbar = 1e-4 * scale
    ...
    def apply_channel_overrides(self):
        if not self.channel_overrides:
            return

        compartment_map = {
            "soma": [self.soma],
            "ais": [self.ais],
            "dend_prox": [self.dend_prox],
            "dend_dist": [self.dend_dist],
            "spine_head": self.spines,
            "spine_neck": self.spine_necks,
        }

        for compartment_name, params in self.channel_overrides.items():
            if compartment_name not in compartment_map:
                continue

            target_sections = compartment_map[compartment_name]

            bk_total_gakbar = None
            bk_total_gabkbar = None

            for key, value in params.items():
                if key == "BK_TOTAL.gakbar":
                    bk_total_gakbar = value
                elif key == "BK_TOTAL.gabkbar":
                    bk_total_gabkbar = value

            for sec in target_sections:
                for seg in sec:
                    for key, value in params.items():
                        if key.startswith("BK_TOTAL."):
                            continue

                        mech_name, field_name = key.split(".", 1)
                        if has_mech(sec, mech_name):
                            mech_obj = getattr(seg, mech_name)
                            if hasattr(mech_obj, field_name):
                                setattr(mech_obj, field_name, value)

                if bk_total_gakbar is not None or bk_total_gabkbar is not None:
                    current_gak = bk_total_gakbar if bk_total_gakbar is not None else 0.0
                    current_gabk = bk_total_gabkbar if bk_total_gabkbar is not None else 0.0
                    apply_bk_split_to_section(
                        sec,
                        total_bk_gakbar=current_gak,
                        total_bk_gabkbar=current_gabk,
                        split=self.bk_split,
                    )
