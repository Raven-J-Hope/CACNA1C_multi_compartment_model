...
#                    seg.BK.gabkbar = 1e-4
    ...
    def _set_biophysics(self):
        ...
        print("[AIS debug] has na8st on AIS?", has_mech(self.ais, "na8st"))
        if has_mech(self.ais, "na8st"):
            print("[AIS debug] AIS na8st.gbar @0.5 =", float(self.ais(0.5).na8st.gbar))
        print("[AIS debug] has na8st on soma?", has_mech(self.soma, "na8st"))
        if has_mech(self.soma, "na8st"):
...
#    plt.show()
    ...
    if cai0_soma is not None and cai1_soma is not None:
        plt.figure()
        plt.plot(t0, cai0_soma, color=WT_COLOR, label=WT_LABEL)
    ...
    if cai0_dist is not None and cai1_dist is not None:
        ...
        plt.xlabel("Time (ms)")
        plt.ylabel("cai (mM)")
        plt.title("Intracellular Ca at dist dend")
        plt.legend()
        plt.tight_layout()
    ...
    if cai0_spine is not None and cai1_spine is not None:
        ...
        plt.xlabel("Time (ms)")
        plt.ylabel("cai (mM)")
        plt.title("Intracellular Ca at spine-site") #is this still spine site?
        plt.legend()
        plt.tight_layout()
    ...
    def run_fi_curve(cav12_factor=1.0, currents=np.arange(0.0, 0.8, 0.05),
        ...
        for amp in currents:
            ...
            (
                ...
                bk_acai12_ais, bk_acai12_prox, bk_acai12_dist, bk_acai12_spine,
                bk_acai21_ais, bk_acai21_prox, bk_acai21_dist, bk_acai21_spine
            ) = run_sim(cell, tstop=tstop, v_init=v_init, dt=dt)
...
plt.plot(vs0, dvdt0, color=WT_COLOR, label=WT_LABEL)
plt.plot(vs1, dvdt1, color=CAV12_50_COLOR, label=CAV12_50_LABEL)
plt.scatter([vrest0], [0.0], color=WT_COLOR, label="WT rest", zorder=5)
plt.scatter([vrest1], [0.0], color=CAV12_50_COLOR, label="Cav1.2 50% rest", zorder=5)
plt.xlabel("V (mV)")
plt.ylabel("dV/dt (mV/ms)")
